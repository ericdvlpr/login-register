<?php
function profile_image($user_id, $file_temp,$file_extn){
...
function update_user($update_data){
	...
	$update = array();
	array_walk($update_data, 'array_sanitize');//sanitize every data
...
function change_password($user_id, $password){
	...
	function register_student($student_data){
		array_walk($student_data, 'data_sanitize');
		$fields = '`'.implode('`,`', array_keys($student_data)). '`';
		$data = '\''. implode('\',\'', $student_data).'\'';
		mysql_query("INSERT INTO `tbl_student` ($fields) VALUES ($data)");
	}
function register_user($register_data){
	array_walk($register_data, 'array_sanitize');//sanitize every data
	...
	email($register_data['email'],'Activate Your account', "
		Hello ".$register_data['first_name'].",\n \n You need to activate your account, so use the link below: \n \n http://localhost/snrhgh_system/activate.php?email=".$register_data['email']."&email_code=".$register_data['email_code']." \n\n -phpacademy
		");
...
function user_id_from_email($email){
	$email = sanitize($email);
	return (mysql_result(mysql_query("SELECT `user_id` FROM `users` WHERE `email`='$email'"), 0) == 1) ? true : false;
}
